...
WITH exploded_values AS (
    SELECT key, explode(values) AS value
    FROM nested_data
)
SELECT key,
       AGGREGATE(collect_list(value),
           (CAST(1.0 AS DOUBLE) AS product, CAST(0 AS INT) AS N),
           (buffer, value) -> (value * buffer.product, buffer.N + 1),
           buffer -> POWER(buffer.product, 1.0 / buffer.N)) AS geomean
